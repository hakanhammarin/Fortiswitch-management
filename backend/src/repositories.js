...
export function setSwitchStatus(id, { reachable, lastError, systemInfo, vlans }) {
  const current = db.prepare(`SELECT vlans_json FROM switch_status WHERE switch_id = ?`).get(id);
  db.prepare(
    `INSERT INTO switch_status (switch_id, reachable, last_poll_at, last_error, system_info_json, vlans_json)
     VALUES (@id, @reachable, datetime('now'), @lastError, @systemInfo, @vlans)
     ON CONFLICT(switch_id) DO UPDATE SET
       reachable=@reachable, last_poll_at=datetime('now'), last_error=@lastError,
       system_info_json=@systemInfo, vlans_json=@vlans`
  ).run({
    id,
...
export function listPorts(switchId) {
  return db.prepare(`SELECT * FROM ports WHERE switch_id = ? ORDER BY port_name`).all(switchId).map((p) => ({
    ...p,
    allowedVlans: p.allowed_vlans ? JSON.parse(p.allowed_vlans) : [],
  }));
}
...
export function listBackups(switchId) {
  return db
    .prepare(`SELECT * FROM backups WHERE switch_id = ? ORDER BY created_at DESC`)
    .all(switchId);
}
